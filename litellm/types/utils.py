...
class GenericGuardrailAPIUsage(BaseModel):
    ...
    prompt_tokens: int
    total_tokens: int
    completion_tokens_details: Optional[GenericGuardrailAPICompletionTokensDetails] = (
        None
    )
...
def _normalize_generic_guardrail_api_completion_tokens_details(
    ...
    details = _get_generic_guardrail_api_usage_field(
        usage, "completion_tokens_details"
    ) or _get_generic_guardrail_api_usage_field(usage, "output_tokens_details")
    if details is None:
        return None
    normalized_details = GenericGuardrailAPICompletionTokensDetails(
        accepted_prediction_tokens=_get_generic_guardrail_api_int_usage_field(
            details, "accepted_prediction_tokens"
        ),
        audio_tokens=_get_generic_guardrail_api_int_usage_field(
            details, "audio_tokens"
        ),
        reasoning_tokens=_get_generic_guardrail_api_int_usage_field(
            details, "reasoning_tokens"
        ),
        rejected_prediction_tokens=_get_generic_guardrail_api_int_usage_field(
            details, "rejected_prediction_tokens"
        ),
    )
    return (
        normalized_details if normalized_details.model_dump(exclude_none=True) else None
    )
...
def _normalize_generic_guardrail_api_prompt_tokens_details(
    ...
    ) or _get_generic_guardrail_api_usage_field(usage, "input_tokens_details")
    if details is None:
        return None
    normalized_details = GenericGuardrailAPIPromptTokensDetails(
        ...
        cached_tokens=_get_generic_guardrail_api_int_usage_field(
...
def normalize_generic_guardrail_api_usage(
    ...
    if isinstance(usage, GenericGuardrailAPIUsage):
        return usage
    prompt_tokens = _get_generic_guardrail_api_int_usage_field(
        usage, "prompt_tokens", "input_tokens"
    )
    completion_tokens = _get_generic_guardrail_api_int_usage_field(
    ...
    return GenericGuardrailAPIUsage(
        ...
        prompt_tokens_details=_normalize_generic_guardrail_api_prompt_tokens_details(
            usage
        ),
        completion_tokens_details=_normalize_generic_guardrail_api_completion_tokens_details(
            usage
        ),
    )
